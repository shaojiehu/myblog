...
const userModel = require('../database/mysql')
const moment = require('moment')
const checkNotLogin = require('./check').checkNotLogin
...
exports.getSinglePosts = async ctx => {
    ...
    await userModel.updateArticlePv(postId)
...
exports.postCreate = async ctx => {
    ...
    await userModel.findUser(ctx.session.user)
        .then(res => {
            avator = res[0]['avator']
        })
    await userModel.insertArticle([name, newTitle, md.render(content), content, id, time, avator])
        .then(() => {
            ctx.body = {
                code:200,
                message:'发表文章成功'
...
exports.postComment = async ctx => {
    ...
    await userModel.addArticleCommentCount(postId)
        .then(() => {
            ctx.body = {
                code:200,
                message:'评论成功'
